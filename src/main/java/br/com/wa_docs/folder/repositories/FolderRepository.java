...
public interface FolderRepository extends JpaRepository<Folder, Long> {
    ...
    @Query("SELECT f FROM Folder f WHERE f.project.id = ?1 AND f.parent.id = ?2 ORDER BY f.name ASC")
    List<Folder> findByProjectIdAndParentId(Long projectId, Long parentId);

    @Query("SELECT f FROM Folder f WHERE f.project.id = ?1 ORDER BY f.name ASC")
